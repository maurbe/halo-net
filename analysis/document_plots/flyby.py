...
def fly(sim):
    ...
    input_box = np.load(homedir + 'boxes' + sim + '/input_density_contrast' + sim + '.npy')
    true_box  = np.load(homedir + 'boxes' + sim + '/gt_distancemap_norm' + sim + '.npy')
    ...
    for box, name, cmp, bounds in zip([input_box, true_box, final_box],
                                      ...
                                      ['magma', 'twilight_r', 'twilight_r'],
                                      [[-3, 3], [0, 1], [0, 1]]):
        fig = plt.figure()
        plt.axis('off')
        ax_epoch = fig.add_axes([0.1, 0.1, 0.8, 0.8])
        img = plt.imshow(box[0], animated=True, cmap=cmp, vmin=bounds[0], vmax=bounds[1])
