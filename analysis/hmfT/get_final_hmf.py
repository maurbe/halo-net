...
ax_inset.text(0.95, 0.85, 'Total: {}'.format(len(ps_pred)), fontsize=10, color='#1565C0', ha='right', va='top', transform = ax_inset.transAxes)
...
ax.plot(x, np.log10(counts_log_pred), '#1565C0', label='Predicted Halos')
ax.fill_between(x, error_low, error_high, color='cornflowerblue', alpha=0.4, linewidth=0)
ax.set_ylabel(r'$\log_{10}$N($>$M)')
ax.set_ylim((-0.25, 3.75))
ax.xaxis.set_ticks_position('none')
ax.legend(loc='upper right', prop={'size': 15}, frameon=False)

ax2.plot(x, (counts_log_pred-counts_log_halo_sizes)/counts_log_halo_sizes, color='#1565C0', linestyle='-')
ax2.fill_between(x,
