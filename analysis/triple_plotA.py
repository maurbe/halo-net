...
input = np.load('boxesA/inputA.npy')
gt = np.load('boxesA/gtA.npy')
pred = np.load('boxesA/predictionA.npy')

# Merger situtuation (?): slice 300 middle left
# 250 has some intersting ones as well
n = 431  # I like: 350, 331
input = input[n]
gt = gt[n]
pred = pred[n]
...
# first row
f, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True, figsize=(11, 8))
