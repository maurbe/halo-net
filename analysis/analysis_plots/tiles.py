...
def colorbar_top(mappable, colorbar_label):
...
def tiles_plot(sim):
    ...
    field = input_density_contrast[:256+1, :448+1, 0]
    im = ax.imshow(field, cmap='magma', clim=(field.min(), field.max()),
                   norm=MidpointNormalize(vmin=field.min(), vmax=field.max(), midpoint=0.0))
    cb = colorbar_top(mappable=im, colorbar_label=r'$\delta_{\mathrm{uvs}}$')
    ...
    #ax.add_patch(matplotlib.patches.Rectangle((16+64, 16+64), 96, 96, linewidth=1.0, edgecolor='k', facecolor='none'))
    ...
    ax.add_patch(matplotlib.patches.Rectangle((16 + 32, 16 + 32), 96, 96, linewidth=1.0, edgecolor='k', facecolor='none'))
