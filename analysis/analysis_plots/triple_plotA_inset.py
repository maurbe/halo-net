...
def inset_plot(IN, GT, PRED, n, xl, xr, yb, yt, key):
    ...
    im2 = ax2.imshow(gt, cmap=mcmap, clim=(0, 1))
    ...
    im3 = ax3.imshow(pred, cmap=mcmap, clim=(0, 1))
    ...
    for ax, field, cm, vMax, vMin in zip([ax1, ax2, ax3], [input, gt, pred], ['magma', mcmap, mcmap],
        ...
        for axis in ['top', 'bottom', 'left', 'right']:
            ...
            axins.spines[axis].set_color('#78909C')

        axins.tick_params(axis=u'both', which=u'both', length=0)
        _patch, pp1, pp2 = mark_inset(ax, axins, 3, 4, fc="none", lw=2.0, ec='#78909C', zorder=0, linestyle=':')
        ...
        rect = patches.Rectangle((xl, yb), xr - xl, yt - yb, linewidth=1.5, edgecolor='#78909C', facecolor='none')
        ax.add_patch(rect)
