...
raw_masses, peak_vals, contour_fs = find_peak_to_thresh_relation(distance=predicted_distances)
np.save('peak_thresh_src/raw_masses.npy', raw_masses)
np.save('peak_thresh_src/peak_vals.npy', peak_vals)
np.save('peak_thresh_src/contour_fs.npy', contour_fs)
"""
...
peak_vals = np.asarray([ m * np.ones(100) for m in raw_masses])
...
for j, (line, tc) in enumerate(zip(hist, true_counts[::-1])):
    ...
    if hist[j, index_of_best_fit] < 50:
        markers.append('d')
    else:
        markers.append('o')
...
x = np.linspace(0.0, max(Pv), 100)
print(x) # <- Todo: problem!
